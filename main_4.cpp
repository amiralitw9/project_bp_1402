                        ...
                        if (mouseX >= p.x[1] && mouseX <= p.x[1] + (width / 3) && mouseY >= p.y[2] && mouseY <= p.y[2] + (height / 9)) {
                            if (true) {//نوع مود بازی
                                ...
                                while (!quit) {
                                    ...
                                    //**********************************
                                    int x = 450 , y = 720 ;//مختصات cannon
                                    int mouseX, mouseY;
                                    SDL_GetMouseState(&mouseX, &mouseY); // Get the current mouse position
                                    mouseX=mouseX*900/1920;
                                    mouseY=mouseY*900/1080;
                                    double angle = atan2(y-mouseY , -mouseX+x ) * 180 / M_PI; // Calculate the angle based on mouse position
                                    SDL_Texture* image = images[21].first; // Get the image from the vector based on its index
                                    SDL_Rect destinationRect = {x, y, 100, 100}; // Define the destination rectangle with dimensions 80x80
                                    SDL_Point center = {50, 60}; // Set the center of rotation to the center of the rectangle
